  ...
  return (
    <FunnelShell>
      <div className="overflow-hidden rounded-2xl bg-surface text-ink shadow-card">
        <div className="relative h-60 bg-ink"><img src={vehicle.image_url} alt={`${vehicle.year} ${vehicle.make} ${vehicle.model}`} className="h-full w-full object-cover" /><div className="absolute inset-0 bg-gradient-to-t from-ink/90 via-transparent to-transparent" /><div className="absolute left-4 top-4 rounded-full bg-amber px-3 py-1 text-xs font-black uppercase tracking-wide text-ink">Draft ad landing page · live crawl data</div><div className="absolute bottom-4 left-5 text-surface"><p className="text-xs font-bold uppercase tracking-[0.18em] text-amber">South Park Nissan · Ancira</p><h1 className="mt-1 text-2xl font-black">{vehicle.year} {vehicle.make} {vehicle.model} {vehicle.trim}</h1></div></div>
