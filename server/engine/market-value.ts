...
export function estimateMarketValue(
  ...
  // Trim outliers: drop comps priced beyond 1.5x IQR so one mispriced
  // listing can't distort the market value every other deal is judged against.
  ...
  if (prices.length >= 5) {
    ...
    const lo = q1 - 1.5 * iqr;
    const hi = q3 + 1.5 * iqr;
    const trimmed = prices.filter((v) => v >= lo && v <= hi);
